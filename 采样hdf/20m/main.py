...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ref_tif_path",
        type=str,
        default="/public/home/xwlin/Data/songyibo/dataset_down_from_GEE/36JWT/S2_remove_cloud/20m/S2_L2A_20240901_20m.tif",
    ...
    if args.ref_tif_path is not None:
        # 从文件名中提取年份，支持格式: S2_L2A_20240901_10m.tif
        basename = os.path.basename(args.ref_tif_path)
        ...
        for part in parts:
            if len(part) == 8 and part.isdigit():
                ...
                break
        if date_str:
            args.year = int(date_str[0:4])
        else:
            # 回退到原始逻辑
            args.year = int(parts[-1][0:4])
    else:
        args.year = None
